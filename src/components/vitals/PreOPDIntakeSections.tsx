      ...
      <div className="p-4">
        {data.length === 0 ? (
          ...
          <div className="border border-gray-200 rounded-lg overflow-hidden">
            {/* Header Row - Adjusted for Notes Column */}
...
> = ({ data, onChange }) => {
  ...
  // New State for "Yes/No" toggle
  const [hasConditions, setHasConditions] = useState(data.length > 0);
  const [isExpanded, setIsExpanded] = useState(data.length > 0);

  const toggleConditions = (val: boolean) => {
    setHasConditions(val);
    setIsExpanded(val);
    if (!val) {
      // Clear data if user selects No
      onChange([]);
      ...
      <div className="p-4">
        <div className="mb-4">
          ...
          <div className="flex space-x-4">
            ...
            <button
              type="button"
              onClick={() => toggleConditions(false)}
              className={`px-6 py-2 rounded-lg border transition-colors font-medium ${
                !hasConditions
        ...
        {isExpanded && hasConditions && (
          <div className="space-y-4 pt-3 border-t border-gray-200">
            {/* Add Condition Button was moved here */}
              ...
              <div className="mb-4 p-3 border border-blue-200 rounded-lg bg-blue-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {MOCK_MASTERS.chronicConditions
                    .filter(
                      (condition) => !data.some((d) => d.name === condition)
                    )
                    .map((condition) => (
                      <button
                        key={condition}
              ...
              <div className="space-y-4">
                {data.map((condition) => {
                  ...
                  const isCustomDuration =
                    typeof condition.duration === "object";
                      ...
                      <div
                        ...
                        onClick={() =>
                          setSelectedCondition(
                            isConditionExpanded ? null : condition.id
                          )
                        }
                      >
                        <div className="flex items-center justify-between">
                                ...
                                <table className="w-full">
                                  <thead className="bg-gray-100">
                                    <tr className="text-md text-gray-600">
                                      ...
                                      <th className="text-left p-2 w-1/5">
                                        Dose
                                      </th>
...
export const PastHistorySection: React.FC<PastHistorySectionProps> = ({
  ...
  // Determine if there is any history data present
  ...
  // New State for "Yes/No" toggle
  const [hasHistory, setHasHistory] = useState(hasData);
  ...
  const toggleHistory = (val: boolean) => {
    ...
    setIsExpanded(val);
    if (!val) {
      // Clear all history data if user selects No
      onChange({
        illnesses: [],
        surgeries: [],
        hospitalizations: [],
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      ...
      <div className="p-4 space-y-6">
        <div className="mb-4">
          <p className="text-lg font-medium text-[#0B2D4D] mb-2">
            Does the patient have relevant past medical history?
          </p>
          <div className="flex space-x-4">
            <button
              type="button"
        ...
        {isExpanded && hasHistory && (
          <div className="space-y-6 pt-3 border-t border-gray-200">
      ...
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            {/* ✅ Renumbered to 8 per user request */}
            <h2 className="text-lg font-semibold text-[#0B2D4D]">
              8. AI Clinical Impressions (Pre-OPD)
            </h2>
          </div>
          <div className="flex items-center space-x-2">
            {summary && (
        ...
        {!summary && !isLoading && (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-lg">
              ...
              impression + reasoning based on vitals and chief complaints.
              Normal findings are summarized as WNL.
            </p>
          </div>
        )}
      ...
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            {/* ✅ Renumbered to 7 per user request logic (History before Impressions) */}
            <h2 className="text-lg font-semibold text-[#0B2D4D]">
              7. Previous Medical History Summary (Records + History)
            </h2>
          </div>
          <div className="flex items-center space-x-2">
            {summary && (
