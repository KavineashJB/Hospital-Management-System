...
export const PreOPDIntake: React.FC<PreOPDIntakeProps> = ({
  ...
  // 1. AI Clinical Summary (Vitals + Complaints + Chronic Conditions + Allergies)
              ...
              (c) =>
                `- ${c.name} (Duration: ${
                  typeof c.duration === "string"
                    ? c.duration
                    : `${c.duration.years} yrs ${c.duration.months} mos`
                }, On Meds: ${c.onMedication})`
            )
            .join("\n")
        : "No chronic conditions recorded.";

    // Include Allergies
    const allergiesText = intakeData.allergies.hasAllergies
      ? `**Yes**. Types: ${intakeData.allergies.type.join(", ")}. Substance: ${
          intakeData.allergies.substance
        }. Reaction: ${intakeData.allergies.reaction}. Severity: ${
          intakeData.allergies.severity
        }.`
      : "No known allergies.";
...
Your task is to generate a concise, clinically appropriate summary using vitals, chief complaints, chronic conditions, and allergies, following EXACTLY the format and behavior rules below.
...
- Use "1.", "2.", etc.
- Each impression should be short, clinical, and based on the synthesis of complaints, vitals, and relevant history (chronic/allergies).

#### 3️⃣ Reason line under each impression:
- Must begin with: **Reason:**
- Provide one single line.
- Include relevant abnormal vital values, key symptoms, severity, and context from chronic conditions or allergies if contributory.
...
- DO NOT echo complaints verbatim; extract only clinically meaningful components.
- If no abnormal vitals and no concerning chief complaint:
  - Use a single impression: "No acute abnormality detected based on current data."
- ALWAYS end the output with: **All other vitals and systems: WNL (Within Normal Limits).**
`.trim(),
              },
              {
                role: "user",
                content: combinedData, // vitals + chief complaints + chronic + allergies
              },
            ],
    ...
    <div className="min-h-screen bg-[#F8F9FA] pb-20">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="space-y-6">
          ...
          {/* 7. Previous Medical History Summary (Records + Past History) - ✅ SWAPPED ORDER */}
          ...
          {/* 8. AI Clinical Summary (Vitals + Complaints + Chronic + Allergies) - ✅ SWAPPED ORDER */}
          <AiClinicalSummarySection
