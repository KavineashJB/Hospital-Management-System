...
interface MedicalDashboardProps {
  ...
  // 🚨 NEW PROPS FOR COMPREHENSIVE AI CONTEXT
  preOpdClinicalSummary?: string;
  preOpdHistorySummary?: string;
  intakeData?: {
    complaints: any[];
...
const MedicalDashboard: React.FC<MedicalDashboardProps> = ({
  ...
  vitals,
  onDiagnosisUpdate,
  preOpdClinicalSummary,
  preOpdHistorySummary,
  intakeData,
}) => {
  ...
  const handleGenerateSuggestions = async () => {
    ...
    if (!OPENAI_API_KEY) {
      alert("Please set VITE_OPENAI_API_KEY in your .env file.");
      return;
    }
...
Do not include any explanatory text or markdown formatting outside of the JSON object.`;
    ...
    const userPrompt = `
      ...
      ${vitals ? JSON.stringify(vitals, null, 2) : "N/A"}

      Pre-OPD AI Summary & History:
      - Clinical Summary: ${preOpdClinicalSummary || "N/A"}
      - History Summary: ${preOpdHistorySummary || "N/A"}
      ...
      ${
        intakeData?.complaints
          .map(
            (c) =>
              `${c.complaint} (${c.duration.value} ${c.duration.unit}) - ${c.severity}`
          )
          .join("; ") || "None"
      }
      ...
      - Illnesses: ${intakeData?.pastHistory.illnesses.join(", ") || "None"}
      - Surgeries: ${
        intakeData?.pastHistory.surgeries.map((s: any) => s.name).join(", ") ||
        "None"
      }
      - Current Meds: ${
      ...
      - Local Examination (Notes): ${consultation.notes || "Not specified"}
    `;
