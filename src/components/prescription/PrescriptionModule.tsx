...
interface PrescriptionModuleProps {
  ...
  consultation: any;
  advice: AdviceData; // 🟢 Receive advice state
  setAdvice: React.Dispatch<React.SetStateAction<AdviceData>>; // 🟢 Receive advice setter
  recentGeneralAdvices: string[]; // 🟢 Receive fetched advices
}

const PrescriptionModule: React.FC<PrescriptionModuleProps> = ({
  selectedPatient,
  consultation,
  advice,
  setAdvice,
  recentGeneralAdvices,
}) => {
  ...
  // 🟢 NEW: Handler to set general advice from chips
  const handleRecentAdviceClick = (recentAdvice: string) => {
    setAdvice((prev) => ({ ...prev, general: recentAdvice }));
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <div ref={containerRef} className="flex flex-col lg:flex-row gap-3">
        {/* General Advice */}
        <div
          className="bg-white rounded-lg border border-gray-200 p-3 h-full flex flex-col"
        ...
        >
          ...
          <textarea
            ...
            className="w-full px-2 py-2 border border-gray-300 rounded-md focus:ring-1 focus:ring-[#1a4b7a] focus:border-transparent text-md resize-none flex-grow"
            rows={4}
            placeholder="Enter general advice for the patient..."
          />
          {/* 🟢 NEW: Recent Advice Chips */}
          {recentGeneralAdvices.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-2">
              <span className="text-xs text-gray-500 font-medium self-center">
                Recent:
              </span>
              {recentGeneralAdvices.map((text, idx) => (
                <button
                  key={idx}
                  onClick={() => handleRecentAdviceClick(text)}
                  className="px-2 py-1 text-xs bg-gray-100 text-[#1a4b7a] border border-gray-200 rounded-full hover:bg-[#e0f7fa] hover:border-[#1a4b7a] transition-colors truncate max-w-[200px]"
                  title={text}
                >
                  {text}
                </button>
